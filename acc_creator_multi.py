#!/usr/bin/env python3
"""
Pump.fun Bulk Account Creator (Multi-threaded)
Usage: python3 acc_creator_multi.py <amount_of_accounts>
"""
...
def log_info(msg, indent=0, thread_id=None):
    if PROGRESS_BAR:
        return
    prefix = "    " * indent
    thread_prefix = f"{Fore.BLUE}[T{thread_id}]{Style.RESET_ALL} " if thread_id is not None else ""
    safe_print(f"{prefix}{thread_prefix}{Fore.CYAN}[*]{Style.RESET_ALL} {msg}")


def log_success(msg, indent=0, thread_id=None):
    if PROGRESS_BAR:
        return
    prefix = "    " * indent
    thread_prefix = f"{Fore.BLUE}[T{thread_id}]{Style.RESET_ALL} " if thread_id is not None else ""
    safe_print(f"{prefix}{thread_prefix}{Fore.GREEN}[+]{Style.RESET_ALL} {msg}")


def log_error(msg, indent=0, thread_id=None):
    if PROGRESS_BAR:
        return
    prefix = "    " * indent
    thread_prefix = f"{Fore.BLUE}[T{thread_id}]{Style.RESET_ALL} " if thread_id is not None else ""
    safe_print(f"{prefix}{thread_prefix}{Fore.RED}[-]{Style.RESET_ALL} {msg}")


def log_warning(msg, indent=0, thread_id=None):
    if PROGRESS_BAR:
        return
    prefix = "    " * indent
    thread_prefix = f"{Fore.BLUE}[T{thread_id}]{Style.RESET_ALL} " if thread_id is not None else ""
    safe_print(f"{prefix}{thread_prefix}{Fore.YELLOW}[!]{Style.RESET_ALL} {msg}")
...
class DomainManager:
    ...
    def get_active_domain(self) -> dict:
        """Get a random active domain (thread-safe)"""
        with self.lock:
    ...
    def increment_error(self, domain: str):
        """Increment error count for a domain (thread-safe)"""
        with self.lock:
            for d in self.domains:
                if d["domain"] == domain:
                    ...
                    if d["error_count"] >= MAX_ERROR_COUNT:
                        d["is_active"] = 0
                        log_warning(f"Domain {Fore.RED}{domain}{Style.RESET_ALL} deactivated (errors >= {MAX_ERROR_COUNT})")
                    self._save_unsafe()
                    break

    def increment_success(self, domain: str):
        """Increment accounts created for a domain (thread-safe)"""
        with self.lock:
            for d in self.domains:
                if d["domain"] == domain:
    ...
    def _save_unsafe(self):
        """Save domains to file (not thread-safe, call within lock)"""
        save_json(DOMAINS_FILE, self.domains)

    def save(self):
        """Save domains to file (thread-safe)"""
        with self.lock:
            self._save_unsafe()

    def get_stats(self) -> dict:
        """Get domain statistics (thread-safe)"""
        with self.lock:
...
class AccountManager:
    ...
    def add_account(self, account: dict):
        """Add account to JSON and optionally to database (thread-safe)"""
        with self.lock:
    ...
    def _save_unsafe(self):
        """Save accounts to file (not thread-safe, call within lock)"""
        save_json(ACCOUNTS_FILE, self.accounts)

    def save(self):
        """Save accounts to file (thread-safe)"""
        with self.lock:
            self._save_unsafe()

    def save_to_database(self, account: dict):
        """Save account to MySQL database (thread-safe)"""
        with self.db_lock:
            try:
...
def create_account_worker(task_id: int, domain_manager: DomainManager, account_manager: AccountManager) -> bool:
    """Create a single pump.fun account (worker function for threading)"""
    global success_count, fail_count

    thread_id = task_id % MAX_CONCURRENCY + 1
    email_fetcher = EmailFetcher()  # Each thread gets its own email fetcher

    # Get active domain
    domain_info = domain_manager.get_active_domain()
    if not domain_info:
        log_error("No active domains available!", thread_id=thread_id)
        with counter_lock:
            fail_count += 1
        return False
    ...
    log_info(f"Email: {Fore.WHITE}{email_addr}{Style.RESET_ALL}", thread_id=thread_id)

    auth = PumpFunAuth()

    # Step 1: Send OTP
    log_info("Sending OTP...", thread_id=thread_id)
    if not auth.init_passwordless(email_addr):
        log_error("Failed to send OTP", thread_id=thread_id)
    ...
    # Step 2: Wait for OTP
    log_info("Waiting for OTP...", thread_id=thread_id)
    otp = email_fetcher.wait_for_otp(email_addr)
    if not otp:
        log_error("OTP not received (timeout)", thread_id=thread_id)
        domain_manager.increment_error(domain)
        with counter_lock:
            fail_count += 1
        return False

    log_success(f"OTP received: {Fore.YELLOW}{otp}{Style.RESET_ALL}", thread_id=thread_id)

    # Step 3: Authenticate
    log_info("Authenticating...", thread_id=thread_id)
    if not auth.authenticate(email_addr, otp):
        log_error("Authentication failed", thread_id=thread_id)
        with counter_lock:
            fail_count += 1
        return False

    # Step 4: Login to pump.fun
    log_info("Logging into pump.fun...", thread_id=thread_id)
    if not auth.pump_login():
        log_error("Pump.fun login failed", thread_id=thread_id)
        with counter_lock:
            fail_count += 1
        return False

    # Step 5: Register
    log_info("Registering...", thread_id=thread_id)
    if not auth.pump_register():
        log_error("Registration failed", thread_id=thread_id)
        with counter_lock:
            fail_count += 1
        return False
    ...
    bio_used = None
    if UPDATE_PROFILE:
        log_info("Updating profile...", thread_id=thread_id)

        # Upload profile image to IPFS
        ipfs_url = None
        if INTERNET_IMAGE:
            # Fetch random image from the internet
            image_bytes, _ = auth.fetch_random_image()
            if image_bytes:
                ipfs_url, _ = auth.upload_image_bytes_to_ipfs(image_bytes)
                if ipfs_url:
                    profile_picture_uploaded = True
                else:
                    log_warning("IPFS upload failed", thread_id=thread_id)
            else:
                log_warning("Image fetch failed", thread_id=thread_id)
        elif os.path.exists(PROFILE_IMAGE_PATH):
            ipfs_url, _ = auth.upload_image_to_ipfs(PROFILE_IMAGE_PATH)
            if ipfs_url:
                profile_picture_uploaded = True
            else:
                log_warning("Image upload failed", thread_id=thread_id)

        # Generate username and update profile
        generated_username = generate_username()
        bio_used = PROFILE_BIO
        success, _ = auth.update_profile(generated_username, PROFILE_BIO, ipfs_url)
        if success:
            log_success(f"Profile: {Fore.YELLOW}{generated_username}{Style.RESET_ALL}", thread_id=thread_id)
        else:
            log_warning("Profile update failed", thread_id=thread_id)

    # Step 7: Follow main profile (if enabled)
    followed = False
    if DO_FOLLOW:
        log_info("Following main profile...", thread_id=thread_id)
        if auth.follow_profile(MAIN_PROFILE):
            log_success(f"Followed {Fore.YELLOW}{MAIN_PROFILE[:8]}...{Style.RESET_ALL}", thread_id=thread_id)
            followed = True
        else:
            log_warning("Follow failed (non-critical)", thread_id=thread_id)

    # Step 8: Send reply (if enabled)
    reply_sent = False
    if SEND_REPLY:
        reply_message = random.choice(RANDOM_REPLIES) if RANDOM_REPLY else REPLY_MESSAGE
        log_info("Sending reply...", thread_id=thread_id)
        success, _ = auth.send_reply(REPLY_COIN, reply_message)
        if success:
            log_success(f"Reply sent to {Fore.YELLOW}{REPLY_COIN[:8]}...{Style.RESET_ALL}", thread_id=thread_id)
            reply_sent = True
        else:
            log_warning("Reply failed", thread_id=thread_id)

    # Success!
    profile_url = f"https://pump.fun/profile/{auth.wallet.public_key}"
    log_success(f"{Fore.GREEN}{Style.BRIGHT}SUCCESS!{Style.RESET_ALL} Wallet: {Fore.YELLOW}{auth.wallet.public_key[:16]}...{Style.RESET_ALL}", thread_id=thread_id)
...
def main():
    ...
    if len(sys.argv) < 2:
        print_banner()
        print(f"{Fore.RED}Usage:{Style.RESET_ALL} python3 acc_creator_multi.py <amount_of_accounts>")
        print(f"{Fore.YELLOW}Example:{Style.RESET_ALL} python3 acc_creator_multi.py 50")
        print(f"\n{Fore.CYAN}Note:{Style.RESET_ALL} MAX_CONCURRENCY is set to {MAX_CONCURRENCY} threads")
        sys.exit(1)

    try:
        amount = int(sys.argv[1])
    except ValueError:
        print(f"{Fore.RED}Error:{Style.RESET_ALL} Amount must be an integer")
        sys.exit(1)
    ...
    log_info(f"Max Concurrency: {Fore.MAGENTA}{MAX_CONCURRENCY} threads{Style.RESET_ALL}")
    print(f"{Fore.CYAN}{'─' * 60}{Style.RESET_ALL}")
    if not PROGRESS_BAR:
        log_info(f"Creating {Fore.WHITE}{amount}{Style.RESET_ALL} accounts using {Fore.MAGENTA}{MAX_CONCURRENCY}{Style.RESET_ALL} threads")
        print(f"{Fore.CYAN}{'─' * 60}{Style.RESET_ALL}")
        print()
    ...
    # Progress bar for progress bar mode
    pbar = None
    if PROGRESS_BAR:
        pbar = tqdm(
            total=amount,
        ...
        pbar.set_postfix_str(f'{Fore.GREEN}✓0{Style.RESET_ALL} {Fore.RED}✗0{Style.RESET_ALL}')

    # Use ThreadPoolExecutor - keep creating until we reach target successes
    with ThreadPoolExecutor(max_workers=MAX_CONCURRENCY) as executor:
        futures = set()

        # Initial batch of tasks
        for _ in range(min(MAX_CONCURRENCY, amount)):
            future = executor.submit(create_account_worker, task_id, domain_manager, account_manager)
            futures.add(future)
            task_id += 1

        # Keep replenishing until we have enough successful accounts
        while success_count < amount:
            ...
            # Update progress bar
            if PROGRESS_BAR and pbar:
                new_successes = success_count - last_success
                if new_successes > 0:
                    pbar.update(new_successes)
            ...
            # Add new tasks to replace completed ones (if we still need more)
            while len(futures) < MAX_CONCURRENCY and success_count + len(futures) < amount + 100:
                # Add buffer of attempts since some may fail
                future = executor.submit(create_account_worker, task_id, domain_manager, account_manager)
                futures.add(future)
                task_id += 1

        # Cancel remaining futures if we've reached our goal
        for future in futures:
            future.cancel()

    if pbar:
        # Final update to make sure we show 100%
        if success_count > last_success:
            pbar.update(success_count - last_success)
        pbar.close()
    ...
    print(f"{Fore.CYAN}[*]{Style.RESET_ALL} Total attempts: {Fore.YELLOW}{success_count + fail_count}{Style.RESET_ALL}")
    print(f"{Fore.CYAN}[*]{Style.RESET_ALL} Time elapsed: {Fore.YELLOW}{elapsed_time:.1f}s{Style.RESET_ALL}")
    if elapsed_time > 0:
        print(f"{Fore.CYAN}[*]{Style.RESET_ALL} Accounts/min: {Fore.MAGENTA}{(success_count / elapsed_time * 60):.1f}{Style.RESET_ALL}")
    print()
